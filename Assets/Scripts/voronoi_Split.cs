...
public class voronoi_Split : MonoBehaviour {
...
//		camera1 = Camera.main.gameObject;
		...
		camera2.AddComponent<Camera> ().depth = 0;
		...
		splitter.transform.localScale = new Vector3 (4, splitterWidth/10, 1);
		splitter.transform.localEulerAngles = Vector3.zero;
		splitter.GetComponent<MeshRenderer> ().receiveShadows = false;
		...
		split.GetComponent<MeshRenderer> ().receiveShadows = false;
...
//				camera2.transform.rotation = Quaternion.Lerp(camera2.transform.rotation, newRot2, Time.deltaTime*5);
				camera2.transform.rotation = Quaternion.Euler(60f,0,0);
			}
...
//		camera1.transform.rotation = Quaternion.Lerp(camera1.transform.rotation, newRot, Time.deltaTime*5);
		camera1.transform.rotation = Quaternion.Euler(60f,0,0);
	}
}
